...
class BannerController extends Controller
{
   ...
   {    
       ...
       ->addColumn('action',function($query){
            ...
            <a data-toggle="tooltip" data-placement="top" title="Banner Detail" class="dropdown-item" href="'.route('admin.cms.banner.update',$query->id).'"><i class="fas fa-landmark mr-1 text-muted font-18 vertical-middle"></i>Change Banner</a>
            </div></div>';
            return $str;
          })
   ...
   public function update(Request $request,$id)
   {
       $banner = Banner::find($id);
        if(!empty($banner))
        {
            return view('admin.cms.banner.update',compact('banner'));
        }
   ...
   public function store(Request $request)
   {
        if($request->ajax())
        {
            ...
            $upate_banner = Banner::where('id','!=',$request->banner_id)->where('order',$attributes['order'])->first();
            ...
            Cache::rememberForever('banners', function (){
                return Banner::where('status','1')->orderBy('order')->get();
            });
