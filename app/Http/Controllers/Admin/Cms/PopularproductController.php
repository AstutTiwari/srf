...
class PopularproductController extends Controller
{
   ...
   {    
        $query = PopularProduct::where('id','!=','0');
        ...
        ->addColumn('action',function($query){
            ...
            <a data-toggle="tooltip" data-placement="top" title="Product Detail" class="dropdown-item" href="'.route('admin.popularproduct.update.view',$query->id).'"><i class="fas fa-landmark mr-1 text-muted font-18 vertical-middle"></i>Update</a>
            </div></div>';
            return $str;
          })
   ...
   public function createView()
   {
       ...
       return view('admin.cms.popular_product.create',compact('category'));
   }
   ...
   public function updateView(Request $request)
   {
        if($request->id)
        {
            $product = PopularProduct::find($request->id);
            if($product)
            {
                $category = ProductBanner::where('status','1')->pluck('title','id')->toArray();
                return view('admin.cms.popular_product.update',compact('product','category'));
            }
    ...
    public function updateStore(Request $request)
    {
        if($request->ajax())
        {
            ...
            $product = PopularProduct::find($request->product_id);
            if(!$product)
            {
                return response()->json(['success' => false, "error" => 'Product does not exist']);
            }
            ...
            if(isset($attributes['banner_image']) && !empty($attributes['banner_image']))
            {
                ...
                @list(, $extension) = explode('/', $type);
                $unique_name = rand().'.'. $extension;
                $filePath = 'popular_product/'. $unique_name;
                Storage::disk('public')->put($filePath, base64_decode($file));
