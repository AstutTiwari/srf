...
@endsection
@section('meta_page_title','Faq List | Tenantden')
@section('content')
                ...
                <div class="card-body leads-table-body">
                    <div class="section-title-top">
                        ...
                        <div class="title-right">
                            ...
                            <a href="{{route('admin.cms.faq.create')}}"  class="btn btn-primary btn-top btn-cancel mr-2"><i class="mdi mdi-plus-circle-outline mr-1"></i>Add A Faq</a>
                        </div>
                    </div>
                    <div class="filter-section rent-filter" style="display: none;">
                        {!! Form::open(array('method'=>'POST','id'=>'search_faq')) !!}
                        <div class="row">
                            ...
                            <div class="col-sm-6 form-group mb-2 col-md-3">
                                {!! Form::select('type', ['1'=>'General','2'=>'Pricing','3'=>'Markiting','4'=>'Rental'], '', ['id'=>'type', 'class' => 'form-control selectpicker',  'title' => 'Select type', 'placeholder'=>'Select Faq Type']); !!}
                            </div>
                    ...
                    <table id="faqTable" class="table dt-responsive nowrap w-100">
                        <thead>
                            <tr>
                                <th>Title</th>
                                <th>Type</th>
...
<script>
  ...
  var initialized = false;
	table_instance = $('#faqTable').DataTable({
    "lengthChange": false,
    "language": {
        "paginate": {
    ...
    'drawCallback': function (oSettings) {
        if (!initialized) {
                $('#search_faq.dataTables_filter').each(function () {
                    initialized = true;
                });
            }
...
{
    l = Ladda.create( document.querySelector('#search_faq .search-button') );
    l.start();
    status = $('#status').val();
    type = $('#type').val();
...
function faqDelete(data=null,id=null)
{
    $.confirm({
        title: 'Confirm!',
        content: 'Do you want to delete the selected faq ?',
        buttons: {
            confirm: {
                text: 'Yes',
