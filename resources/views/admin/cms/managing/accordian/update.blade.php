                ...
                <div class="card-body">
                	<div class="row">
                		<div class="banner-page-section col-md-12">             	
		                    <form id="homepage-manage_accordian">
		                        <div class="settings-form">
		                        	<div class="row no-margin">
										<div class="mb-3 col-md-6">
		                                  <label class="col-form-label" for="text">Title<span class="required">*</span></label>
											<input type="hidden" name="accordian_id" value="{{@$manage_accordian->id}}">
                                            <input type="text" class="form-control required" name="title" id="title" value="{{@$manage_accordian->title}}">
											<label for="title" id="title-error" generated="true" class="is-invalid" style="display:none"></label>
		                                </div>
                                        <div class="mb-3 col-md-6">                                        
                                            <label class="col-form-label" for="order">Order<span class="required">*</span></label>
                                            {!! Form::select('order', ['1'=>'First','2'=>'Second','3'=>'Third','4'=>'Fourth','5'=>'Fifth'], @$manage_accordian->order, ['id'=>'order', 'class' => 'form-control selectpicker',  'placeholder' => 'Select Order']); !!}
                                        ...
                                        <div class="mb-3 col-md-12">
		                                  <label class="col-form-label" for="text">Managing Text<span class="required">*</span></label>
											<textarea class="form-control required" name="text" id="text">{{@$manage_accordian->text}}</textarea>
...
var l;
$('#homepage-manage_accordian').validate({
...
submitHandler: function(form) {
    ...
    l = Ladda.create( document.querySelector('#homepage-manage_accordian .btn-submit') );
    l.start();
    for (instance in CKEDITOR.instances) {
        CKEDITOR.instances[instance].updateElement();
    }
    $.ajax({
        url: "{{route('admin.manage.accordian.store')}}",
        method: "POST",
        dataType: 'json',
        data: $("#homepage-manage_accordian").serialize(),
        success: function (resultData) {
            ...
            else if(resultData.success)
            {
                ...
                window.location.replace("{{route('admin.manage.accordian')}}");	    }
        },
